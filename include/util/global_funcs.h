...
// reads interpolated element from a uchar* array SSE2 optimization possible
...
// Get interpolated [intensity, gx, gy]
...
// [0]: 内插出的intensity
// [1]: 水平方向的梯度(right - left)
// [2]: 垂直方向的梯度(bottom - top)
...
EIGEN_ALWAYS_INLINE Eigen::Vector2f getInterpolatedElement32Cub(
    const Eigen::Vector3f* const p, const float x) {
  // for x=0, this returns p[1].
