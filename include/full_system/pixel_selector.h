...
// grads[0]: intensity
// grads[1]: gradient x (gx)
// grads[2]: gradient y (gy)
template <int pot>
...
inline int makePixelStatus(Eigen::Vector3f* grads, bool* map, int w, int h,
  ...
  float quotia = static_cast<float>(numGoodPoints) / desiredDensity;
