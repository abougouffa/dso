...
void FrameHessian::setStateZero(const Vec10& state_zero) {
  CHECK_LT(state_zero.head<6>().squaredNorm(), 1e-20);
...
void FrameHessian::release() {
  // DELETE POINT
  // DELETE RESIDUAL
  for (size_t i = 0; i < pointHessians.size(); ++i) {
    delete pointHessians[i];
  }
  for (size_t i = 0; i < pointHessiansMarginalized.size(); ++i) {
    delete pointHessiansMarginalized[i];
  }
  for (size_t i = 0; i < pointHessiansOut.size(); ++i) {
    delete pointHessiansOut[i];
  }
  for (size_t i = 0; i < immaturePoints.size(); ++i) {
