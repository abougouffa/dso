...
#include "optimization_backend/accumulated_sc_hessian_sse.h"
...
void EnergyFunctional::setAdjointsF(CalibHessian *Hcalib) {
  ...
  for (int h = 0; h < nFrames; ++h) {
    for (int t = 0; t < nFrames; ++t) {
      ...
      // (Tong) TODO: maybe Jakob forgot sth...
      // AH(6, 7) = -affLL[0] * host->aff_g2l_0().b;
      // AT(6, 7) = affLL[0] * host->aff_g2l_0().b;
...
void EnergyFunctional::accumulateAF_MT(MatXX &H, VecX &b, const bool MT) {
  if (MT) {
    red->reduce(boost::bind(&AccumulatedTopHessianSSE::setZero, accSSE_top_A,
                            nFrames, boost::placeholders::_1,
    ...
    for (EFFrame *f : frames) {
      for (EFPoint *p : f->points) {
        accSSE_top_A->addPoint<0>(p, this); // mode: active
      }
    }
...
void EnergyFunctional::accumulateLF_MT(MatXX &H, VecX &b, const bool MT) {
  if (MT) {
    ...
    for (EFFrame *f : frames) {
      for (EFPoint *p : f->points) {
        accSSE_top_L->addPoint<1>(p, this); // mode: linearized
      }
    }
...
void EnergyFunctional::accumulateSCF_MT(MatXX &H, VecX &b, const bool MT) {
  if (MT) {
...
void EnergyFunctional::resubstituteF_MT(VecX x, CalibHessian *HCalib, bool MT) {
  CHECK_EQ(x.size(), CPARS + nFrames * 8);
...
void EnergyFunctional::resubstituteFPt(const VecCf &xc, Mat18f *xAd, int min,
  ...
  for (int k = min; k < max; ++k) {
    ...
    CHECK(std::isfinite(p->data->step));
  }
}

double EnergyFunctional::calcMEnergyF() {
  CHECK(EFDeltaValid);
  CHECK(EFAdjointsValid);
...
double EnergyFunctional::calcLEnergyF_MT() {
  ...
  red->reduce(boost::bind(&EnergyFunctional::calcLEnergyPt, this,
                          boost::placeholders::_1, boost::placeholders::_2,
                          boost::placeholders::_3, boost::placeholders::_4),
              0, allPoints.size(), 50);
...
EFFrame *EnergyFunctional::insertFrame(FrameHessian *fh, CalibHessian *Hcalib) {
  ...
  eff->idx = frames.size();
  LOG(WARNING) << "Frame size: " << frames.size();
  frames.emplace_back(eff);

  ++nFrames;
  fh->efFrame = eff;

  CHECK_EQ(HM.cols(), 8 * nFrames + CPARS - 8);
  //  Add new frame state, pose(3), rotation(3), ai(1), bi(1).
...
void EnergyFunctional::dropResidual(EFResidual *r) {
  EFPoint *p = r->point;
  CHECK_EQ(r, p->residualsAll[r->idxInAll]);
...
void EnergyFunctional::marginalizeFrame(EFFrame *fh) {
  ...
  if ((int)fh->idx != (int)frames.size() - 1) {
    ...
    CHECK_EQ(io + 8 + ntail, nFrames * 8 + CPARS);
  ...
  CHECK_EQ(frames.size() * 8 + CPARS, HM.rows());
  CHECK_EQ(frames.size() * 8 + CPARS, HM.cols());
  CHECK_EQ(frames.size() * 8 + CPARS, bM.size());
  CHECK_EQ(frames.size(), nFrames);
...
void EnergyFunctional::marginalizePointsF() {
  ...
  // By default:
  // setting_solverMode:             1000 1000 0000
  // SOLVER_ORTHOGONALIZE_POINTMARG: 0000 0000 0100
  // &=                              0000 0000 0000
  if (setting_solverMode & SOLVER_ORTHOGONALIZE_POINTMARG) {
    // We will never come into this part by default
  ...
  // By default:
  // setting_solverMode:        1000 1000 0000
  // SOLVER_ORTHOGONALIZE_FULL: 0000 0000 1000
  // &=                         0000 0000 0000
...
void EnergyFunctional::solveSystemF(const int iteration, double lambda,
                                    CalibHessian *const HCalib) {
  // By default:
  // setting_solverMode: 1000 1000 0000
  // SOLVER_USE_GN:      0000 0100 0000
  // &=                  0000 0000 0000
  ...
  // By default:
  // setting_solverMode: 1000 1000 0000
  // SOLVER_FIX_LAMBDA:  0000 1000 0000
  // &=                  0000 1000 0000
  if (setting_solverMode & SOLVER_FIX_LAMBDA) {
    // This lambda will be used!
    lambda = 1e-5;
  }
  ...
  // By default:
  // setting_solverMode:          1000 1000 0000
  // SOLVER_ORTHOGONALIZE_SYSTEM: 0000 0000 0010
  // &=                           0000 0000 0000
  ...
  if (setting_solverMode & SOLVER_SVD) {
    ...
    // We will work in this part by default
